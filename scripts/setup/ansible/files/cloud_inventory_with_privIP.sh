...
function writeToFile {
    echo -e "$hostname ansible_user='$User' ansible_ssh_host=$priIP ansible_ssh_private_key_file=~/.ssh/id_rsa" >> $1
}
...
#		priIP="$(echo -e "$novashowreturn" | 'grep' "private network" | 'awk' '{ print $5}' )"
        ...
        if [[ $hostname == *"spark_slaves"* ]]
               then
                       sed -i '/\[spark_slaves\]/a '$hostname' ansible_user='$User' ansible_ssh_host='$priIP' ansible_ssh_private_key_file=~/.ssh/id_rsa' $AnsibleHostFile
                        sed -i '/\[spark_slaves\]/a '$hostname' ansible_ssh_host='$priIP' ansible_ssh_private_key_file=~/.ssh/id_rsa' $AnsibleHostFileBootStrap
            sed -i '/export '$hostname'/c\export '$hostname'='$priIP'' ~/.bashrc
                echo -e "$priIP $hostname " >> $AnsibleHostList
               else
                        echo -e "$hostname ansible_user=$User ansible_ssh_host=$priIP ansible_ssh_private_key_file=~/.ssh/id_rsa" >> $AnsibleHostFile
                echo -e "$hostname ansible_ssh_host=$priIP ansible_ssh_private_key_file=~/.ssh/id_rsa" >> $AnsibleHostFileBootStrap
            sed -i '/export '$hostname'/c\export '$hostname'='$priIP'' ~/.bashrc
                echo -e "$priIP $hostname " >> $AnsibleHostList
        ...
        if [ "$priIP" ]; then
            ssh-keyscan  $priIP >> ~/.ssh/known_hosts
        fi
